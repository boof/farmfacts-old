APPLICATION_NAME    = 'FarmFacts'

MAILING_LISTS       = 'alpha@mailing-list, beta@mailing-list'
SERVER_RECIPIENT    = %w[ admin@host ]
SERVER_SENDER       = %w[ system@host ]

STATIC_HTML         = /\/(?:404|422|500)\.html$/
ASSETS              = %w[ css flv gif jpeg jpg js png swf exe cgi pl ]

DEFAULT_TITLE       = 'Welcome'
DEFAULT_KEYWORDS    = 'ruby, farmfacts, rails, ror, cms, fork'
DEFAULT_DESCRIPTION = 'FarmFacts is a Content Management System based on Ruby on Rails 2.1.1.'